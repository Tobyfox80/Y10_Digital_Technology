...
int code[4] = {9,3,6,1};//code exists, 4 digits
int currentdigit = 0;
int correctdigits = 0;
bool locked = true;
...
void loop() {
  ...
  if(keyset1 == 1023){
      ...
      lcd.clear();
      lcd.setCursor(0, 0);
      if (currentdigit == 4 && correctdigits == 4){//if correct
      locked = false;
      lcd.print("Code correct!");
      //move servo
      delay(1000);
      }
      else if(locked == false){//if pressing when unlocked
        locked = true;
  ...
  if(input !=10){ 
    if(settingcode == false){
      if(input == code[currentdigit]){
        correctdigits ++;
      }
    }
    else if (settingcode == true){
      if(digitsset <= 4){
      code[currentdigit] = input;
      digitsset ++;
      }
      else{
        digitsset = 0;
